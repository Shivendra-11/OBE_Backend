...
const getTeacherSectionsForCourse = (course, teacherId) => {
  ...
  const st = Array.isArray(course.sectionTeachers) ? course.sectionTeachers : [];
  for (const entry of st) {
    if (entry && entry.teacher && entry.teacher.toString() === String(teacherId)) {
      if (entry.section != null) sections.push(String(entry.section));
    }
  }
  // Legacy single-teacher assignment => allow without section constraint
  if (sections.length === 0 && course.assignedTeacher && course.assignedTeacher.toString() === String(teacherId)) {
    return ["__LEGACY__"]; // special marker
  }
  return sections;
};
...
const ensureTeacherAssignedToCourse = async (req, course) => {
  ...
  if (!sections.length) return { ok: false, status: 403, message: "Forbidden: not assigned to this course" };
  return { ok: true, role: "teacher", sections };
};

const resolveTeacherSection = ({ role, sections }, requestedSection) => {
  if (role === "admin") return { ok: true, section: normalizeSection(requestedSection) };
  // legacy assignment: no section restriction possible
  if (sections && sections.length === 1 && sections[0] === "__LEGACY__") return { ok: true, section: null };

  const normalized = normalizeSection(requestedSection);
  if (normalized) {
    const allowed = (sections || []).some(s => String(s).toLowerCase() === normalized.toLowerCase());
    if (!allowed) return { ok: false, status: 403, message: "Forbidden: not assigned to this section" };
    return { ok: true, section: normalized };
  }

  if (!sections || sections.length === 0) return { ok: false, status: 403, message: "Forbidden" };
  if (sections.length > 1) return { ok: false, status: 400, message: "section required (assigned to multiple sections)" };
...
exports.createExamWithQuestions = async (req, res) => {
  try {
    ...
    if ((!courseId && !courseCode) || !name || !type || !Array.isArray(questions) || questions.length === 0) {
      return res.status(400).json({ message: "courseId/courseCode, name, type and questions[] required" });
    }
    ...
    const docs = questions
      ...
      .map(q => ({ exam: exam._id, CO: Number(q.CO), maxMarks: Number(q.maxMarks) }));

    if (docs.length === 0) {
      return res.status(400).json({ message: "No valid questions" });
    }

    const createdQuestions = await Question.insertMany(docs, { ordered: false });
    res.status(201).json({ exam, count: createdQuestions.length, questions: createdQuestions });
  } catch (err) {
    console.error(err);
...
exports.createQuestionBulk = async (req, res) => {
  try {
    ...
    if (!examId || !Array.isArray(questions) || questions.length === 0) {
      return res.status(400).json({ message: "examId and questions array required" });
    }
    ...
    if (docs.length === 0) return res.status(400).json({ message: "No valid questions" });

    const created = await Question.insertMany(docs, { ordered: false });
    res.status(201).json({ count: created.length, examId: exam._id, data: created });
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: "Server error", error: err.message });
  }
...
exports.enterMarks = async (req, res) => {
  try {
    ...
    if (!first) return res.status(400).json({ message: "At least one questionId required" });

    const firstQuestion = await Question.findById(first.questionId).populate({ path: "exam", select: "course" });
    if (!firstQuestion || !firstQuestion.exam) return res.status(400).json({ message: "Invalid questionId" });
    ...
    if (!sectionPick.ok) return res.status(sectionPick.status).json({ message: sectionPick.message });
    ...
    for (const e of entries) {
      ...
      const q = await Question.findById(questionId).populate({ path: "exam", select: "course" });
      if (!q || !q.exam) continue;
      if (q.exam.course.toString() !== course._id.toString()) {
        return res.status(400).json({ message: "All entries must belong to the same course" });
      }
      ...
      if (authz.role !== "admin" && section) {
        const stu = await User.findById(studentId).select("role semester section academicYear").lean();
        if (!stu || stu.role !== "student") return res.status(400).json({ message: "Invalid student" });
        if (stu.section == null) return res.status(400).json({ message: "Student section not set" });
        if (String(stu.section).toLowerCase() !== String(section).toLowerCase()) {
          return res.status(403).json({ message: "Forbidden: student not in your section" });
        }
      }
...
exports.calculateExamCO = async (req, res) => {
  try {
    ...
    await attainmentService.calculateExamCO(exam, { section: sectionPick.section });
    res.json({ message: "Exam CO attainment calculated", section: sectionPick.section || null });
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: "Server error" });
  }
...
exports.calculateCTFinal = async (req, res) => {
  try {
    ...
    await attainmentService.calculateCTFinal(courseId, { section: sectionPick.section });
    res.json({ message: "CT_FINAL calculated", section: sectionPick.section || null });
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: "Server error" });
  }
...
exports.calculateAssignmentFinal = async (req, res) => {
  try {
    ...
    await attainmentService.calculateAssignmentFinal(courseId, { section: sectionPick.section });
    res.json({ message: "ASSIGNMENT_FINAL calculated", section: sectionPick.section || null });
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: "Server error" });
  }
...
exports.calculateOverall = async (req, res) => {
  try {
    ...
    await attainmentService.calculateOverall(courseId, { section: sectionPick.section });
    res.json({ message: "OVERALL calculated", section: sectionPick.section || null });
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: "Server error" });
  }
...
exports.getExamCOAttainment = async (req, res) => {
  try {
    ...
    const records = await COAttainment.find({ exam: exam._id, section: sectionPick.section || null })
      .sort({ CO: 1 })
      .lean();

    res.json({ examId: exam._id, courseId: course._id, section: sectionPick.section || null, data: records });
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: "Server error" });
  }
...
exports.getCourseAttainment = async (req, res) => {
  try {
    ...
    res.json({ courseId: course._id, section: sectionPick.section || null, data: records });
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: "Server error" });
  }
...
exports.getExamMarksheet = async (req, res) => {
  try {
    ...
    const questionsRaw = await Question.find({ exam: exam._id }).select("_id CO maxMarks").lean();
    // Stable ordering: ObjectId time ordering is good enough here.
    const questionsSorted = questionsRaw.sort((a, b) => String(a._id).localeCompare(String(b._id)));
    ...
    const existingMarks = await StudentMark.find({ student: { $in: studentIds }, question: { $in: questionIds } })
    ...
    res.json({
      exam: { id: exam._id, name: exam.name, type: exam.type, course: exam.course },
      course: { id: course._id, code: course.code, name: course.name, semester: course.semester, academicYear: course.academicYear },
...
exports.submitExamMarksheet = async (req, res) => {
  try {
    ...
    const questionsRaw = await Question.find({ exam: exam._id }).select("_id maxMarks").lean();
    const questionsSorted = questionsRaw.sort((a, b) => String(a._id).localeCompare(String(b._id)));
    ...
    const { entries } = req.body;
    const raw = Array.isArray(entries) ? entries : (Array.isArray(req.body) ? req.body : null);
    ...
    if (ops.length === 0) return res.status(400).json({ message: "No valid marks to update" });
