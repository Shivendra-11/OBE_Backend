...
exports.createPOBulk = async (req, res) => {
  try {
    ...
    const docs = pos
      .filter(p => p && p.code)
      .map(p => ({ code: String(p.code), description: p.description == null ? undefined : String(p.description) }));

    if (docs.length === 0) return res.status(400).json({ message: "No valid PO entries" });
...
exports.createCourse = async (req, res) => {
  try {
    ...
    if (!code || !name || semester == null || !academicYear) {
      return res.status(400).json({ message: "code, name, semester and academicYear required" });
    }

    const course = await Course.create({ code, name, semester, academicYear, sectionTeachers: [] });

    // Auto-add this course to all students of the same semester & academic year
    await User.updateMany(
      { role: "student", semester: Number(semester), academicYear: String(academicYear) },
...
exports.createCOBulk = async (req, res) => {
  try {
    ...
    if ((!courseId && !courseCode) || !Array.isArray(cos) || cos.length === 0) {
      return res.status(400).json({ message: "courseId/courseCode and cos array required" });
    }
    ...
    const docs = cos
      ...
      .map(c => ({ course: courseId, number: Number(c.number), description: c.description == null ? undefined : String(c.description) }));

    if (docs.length === 0) return res.status(400).json({ message: "No valid CO entries" });
...
exports.mapCOPoBulk = async (req, res) => {
  try {
    ...
    if ((!courseId && !courseCode) || !Array.isArray(mappings) || mappings.length === 0) {
      return res.status(400).json({ message: "courseId/courseCode and mappings array required" });
    }
    ...
    const docs = mappings
      ...
      .map(m => ({ course: courseId, CO: Number(m.CO), PO: String(m.PO), level: m.level == null ? undefined : Number(m.level) }));

    if (docs.length === 0) return res.status(400).json({ message: "No valid mappings" });
...
exports.assignTeacherToCourse = async (req, res) => {
  try {
    ...
    if ((!courseId && !courseCode) || !teacherId || !section) {
      return res.status(400).json({ message: "courseId/courseCode, teacherId and section required" });
    }
    if (!courseId && courseCode) {
      const c = await Course.findOne({ code: courseCode });
      if (!c) return res.status(404).json({ message: "Course not found for provided courseCode" });
    ...
    if (!normalizedSection) return res.status(400).json({ message: "section required" });

    course.sectionTeachers = Array.isArray(course.sectionTeachers) ? course.sectionTeachers : [];
    const idx = course.sectionTeachers.findIndex(st => String(st.section).toLowerCase() === normalizedSection.toLowerCase());
    if (idx >= 0) {
      course.sectionTeachers[idx].teacher = teacherId;
    } else {
      course.sectionTeachers.push({ section: normalizedSection, teacher: teacherId });
    }
...
exports.listCourses = async (req, res) => {
  try {
    const courses = await Course.find().select('code name semester academicYear assignedTeacher sectionTeachers');
    res.json(courses);
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: 'Server error' });
...
exports.getExamCOAttainment = async (req, res) => {
  try {
    ...
    const records = await COAttainment.find({ exam: exam._id, section: section })
      .sort({ CO: 1 })
      .lean();

    res.json({ examId: exam._id, courseId: exam.course, section: section, data: records });
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: "Server error" });
  }
